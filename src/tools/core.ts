    ...
    async () => {
      ...
      return jsonResponse({
        ...
        index: {
          ...
          parse_error_count: graph.getCatalogIssues().filter(
            (issue) => issue.code === "FRONTMATTER_PARSE_ERROR",
          ).length,
          issue_count: graph.getCatalogIssues().length,
          warning_counts: graph.getWarningCounts(),
          extraction_profile: graph.extractionProfile,
          persisted_snapshot_valid: graph.persistedSnapshotValid,
        },
